...
// compile command: g++ -std=c++17 -Wall -Wextra -pedantic-errors -Weffc++ -fsanitize=undefined,address *.cpp
/*NOTES:
- Line 1 of txt file is the number of rows and columns, respectively, that the map contains
- Line 2 consists of the players starting location, players row and column, respectively
- the rest of the lines are the maps format
- the first value in line 3 is (0,0)
- there are no spaces between the values in the map, they are only there for aesthetic purposes
- row major order: [col][row]
- in a for loop: col will be the outer loop, row will be the inner loop
- only the final level of the dungeon will have an exit "!" symbol
*/
// do not change
